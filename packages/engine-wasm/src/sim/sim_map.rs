/// Named scalar maps for the simulation pipeline.
/// Each map is a flat Vec<f32> of width*height tiles.
...
pub struct SimMapRegistry {
    ...
    active: usize, // which index is "current" (0 or 1)
    width: usize,
    height: usize,
}
...
impl SimMapRegistry {
    pub fn new(width: usize, height: usize) -> Self {
        ...
        // Construct each pair as two zero-filled vecs.
        // We build an array of [Vec<f32>; 2] for each of SIM_MAP_COUNT slots.
    ...
    /// Zero-fill the next (write) buffer for the given map.
...
mod tests {
    ...
    fn new_creates_zero_filled_buffers() {
        ...
        for &map in &[
            SimMap::Power,
            SimMap::ServiceHealth,
            SimMap::ServiceSafety,
            SimMap::ServiceEduc,
            SimMap::TrafficDensity,
            SimMap::Pollution,
            SimMap::LandValue,
            SimMap::ZoneDemand,
        ] {
            assert!(
                reg.current(map).iter().all(|&v| v == 0.0_f32),
                "expected zeros for {:?}",
                map
            );
        }
    ...
    fn swap_makes_written_next_become_current() {
        ...
        // Write a sentinel value into next buffer.
        let next = reg.next_mut(SimMap::Power);
        next[0] = 42.0_f32;

        // Before swap, current should still be zeros.
        assert_eq!(reg.current(SimMap::Power)[0], 0.0_f32);

        reg.swap();

        // After swap, current is the buffer we just wrote.
    ...
    fn clear_next_zeroes_next_buffer() {
        ...
        // clear_next should zero them.
        reg.clear_next(SimMap::Pollution);

        // Swap so we can read what was the next buffer.
        reg.swap();
        assert!(
            reg.current(SimMap::Pollution).iter().all(|&v| v == 0.0_f32),
            "clear_next did not zero the buffer"
        );
    }
}
